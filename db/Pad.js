...
// var hooks = require('../ep_etherpad-lite/pluginfw/hooks');
...
Pad.prototype.appendRevision = function appendRevision(aChangeset, author) {
  ...
  db.init(function(err){
    db.set("pad:"+_this.id+":revs:"+newRev, newRevData);
  });
  this.saveToDatabase();
...
Pad.prototype.getLastEdit = function getLastEdit(callback){
  ...
  var _this  = this;
  db.init(function(err){
    db.getSub("pad:"+_this.id+":revs:"+revNum, ["meta", "timestamp"], callback);
  });
}
...
Pad.prototype.getRevisionChangeset = function getRevisionChangeset(revNum, callback) {
  ...
  var _this  = this;
  db.init(function(err){
    db.getSub("pad:"+_this.id+":revs:"+revNum, ["changeset"], callback);
  });
};
...
Pad.prototype.getRevisionAuthor = function getRevisionAuthor(revNum, callback) {
  ...
  var _this  = this;
  db.init(function(err){
    db.getSub("pad:"+_this.id+":revs:"+revNum, ["meta", "author"], callback);
  });
};
...
Pad.prototype.getRevision = function getRevisionChangeset(revNum, callback) {
  ...
  var _this  = this;
  db.init(function(err){
    db.get("pad:"+_this.id+":revs:"+revNum, callback);
  })
};
...
Pad.prototype.appendChatMessage = function appendChatMessage(text, userId, time) {
  ...
  var _this  = this;
  db.init(function(err){
    db.set("pad:"+_this.id+":chat:"+this.chatHead, {"text": text, "userId": userId, "time": time});
  })
  this.saveToDatabase();
};
...
Pad.prototype.init = function init(text, callback) {
  ...
  //try to load the pad
  db.init(function(err){
    db.get("pad:"+_this.id, function(err, value)
    {
      if(ERR(err, callback)) return;
