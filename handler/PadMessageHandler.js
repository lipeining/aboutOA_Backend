...
var Changeset = require("../ep_etherpad-lite/Changeset");
var AttributePool = require("../ep_etherpad-lite/AttributePool");
var AttributeManager = require("../ep_etherpad-lite/AttributeManager");
...
exports.handleMessage = function (client, message) {
  console.log('on handleMessage sessionINfo is');
  console.log(sessioninfos);
  console.log('on padMessageHandler the client.request.session is');
...
function handleSaveRevisionMessage(client, message) {
  ...
  console.log('handleSaveRevisionMessage');
...
exports.sendChatMessageToPadClients = function (time, userId, text, padId) {
  var pad;
  var userName;
  console.log("sendChatMessageToPadClients");;
  async.series([
    //get the pad
    function (callback) {
...
function handleGetChatMessages(client, message) {
  console.log("handleGetChatMessges");
...
function handleSuggestUserName(client, message) {
  console.log("handleSuggestUsername");
  //check if all ok
...
function handleUserInfoUpdate(client, message) {
  console.log("handleUserInfoUpdate");
  //check if all ok
...
function handleUserChanges(data, cb) {
  ...
  async.series([
      ...
      async.whilst(
        ...
        function (callback) {
          ...
          pad.getRevisionChangeset(r, function (err, c) {
            ...
            console.log('handle revision changeset like follow' + util.inspect(changeset));
    ...
    function (callback) {
      ...
      console.log('pad.appendRevision' + util.inspect(changeset));
      var correctionChangeset = _correctMarkersInPad(pad.atext, pad.pool);
      if (correctionChangeset) {
        pad.appendRevision(correctionChangeset);
      }
      console.log('pad.appendRevision _correctMarkersInPad' + util.inspect(correctionChangeset));
      // Make sure the pad always ends with an empty line.
      if (pad.text().lastIndexOf("\n") != pad.text().length - 1) {
        var nlChangeset = Changeset.makeSplice(pad.text(), pad.text().length - 1,
          0, "\n");
        pad.appendRevision(nlChangeset);
        console.log('pad.appendRevision with an empty line' + util.inspect(nlChangeset));
      }
  ...
  async.series([
    ...
    function (callback) {
      ...
      securityManager.checkAccess(padIds.padId, message.sessionID, message.token, message.password, function (err, statusObject) {
        if (ERR(err, callback)) return;
        console.log('securityManager return:' + util.inspect(statusObject));
        console.log('padIds:' + util.inspect(padIds));
        //access was granted
        if (statusObject.accessStatus == "grant") {
    ...
    function (callback) {
      console.log('get the pad:' + util.inspect(pad));
...
function handleChangesetRequest(client, message) {
  console.log("handleChangesetRequest");
  //check if all ok
